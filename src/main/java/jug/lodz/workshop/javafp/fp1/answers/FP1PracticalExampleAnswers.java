package jug.lodz.workshop.javafp.fp1.answers;
 ...
 */
public class FP1PracticalExampleAnswers {
    static private Map<Integer,Purchase> database=initDatabase();

    public static Optional<Purchase> findPurchase(Integer id){
        Purchase result = database.get(id);
        return Optional.ofNullable(result);
    }

    static Function<Purchase,Collection<BigDecimal>> domainFunction =
            p->p.getPurchasedProducts().stream().map(prod->prod.price).collect(Collectors.toList());

    static Function<Collection<BigDecimal>, BigDecimal> genericMathFunction=c->
            c.stream().reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
